...
# vc_id mapping for D4XX sensor entity to IPU6 CSI-2 pad.
...
# vc_id mapping for D4XX sensor entity to IPU7 CSI-2 pad.
# IPU7 CSI-2 pad  1, vc 0, depth a-?
...
# DS5 MUX. Can be max96724 aggregated {a, b, c, d} accross mipi port {0, 1, 2}.
for camera in ${mux_list}; do
  ...
  [[ $quiet -eq 0 ]] && echo -n "Bind DS5 mux ${camera} .. "
  ...
  out $media_ctl_cmd -v -l "\"Intel IPU7 CSI2 ${csi2}\":$((${isys_pad}+1)) -> \"Intel IPU7 ISYS Capture $((${cap_pad}+0))\":0[1]" 1>/dev/null
  # DEPTH metadata node
  if [[ $metadata_enabled -eq 1 ]]; then
     out $media_ctl_cmd -v -l "\"Intel IPU7 CSI2 ${csi2}\":$((${isys_pad}+2)) -> \"Intel IPU7 ISYS Capture $((${cap_pad}+1))\":0[1]" 1>/dev/null
  fi

  # RGB link video streaming node (no metadata)
  out $media_ctl_cmd -v -l "\"Intel IPU7 CSI2 ${csi2}\":$((${isys_pad}+3)) -> \"Intel IPU7 ISYS Capture $((${cap_pad}+2))\":0[1]" 1>/dev/null
  # RGB metadata node
  if [[ $metadata_enabled -eq 1 ]]; then
      out $media_ctl_cmd -v -l "\"Intel IPU7 CSI2 ${csi2}\":$((${cap_pad}+4)) -> \"Intel IPU7 ISYS Capture $((${cap_pad}+3))\":0[1]" 1>/dev/null
  fi
  ...
  out $media_ctl_cmd -v -l "\"Intel IPU7 CSI2 ${csi2}\":$((${isys_pad}+5)) -> \"Intel IPU7 ISYS Capture $((${cap_pad}+4))\":0[1]" 1>/dev/null
  # IMU link streaming node
  out $media_ctl_cmd -v -l "\"Intel IPU7 CSI2 ${csi2}\":$((${isys_pad}+6)) -> \"Intel IPU7 ISYS Capture $((${cap_pad}+5))\":0[1]" 1>/dev/null
  ...
  out $media_ctl_cmd -v -l "\"DS5 mux ${mux}\":0 -> \"Intel IPU7 CSI-2 ${csi2}\":0[1]" 1>/dev/null

  # subdev entity '['pad-number '/' stream-number '->' pad-number '/' stream-number '[' route-flags ']' ']' ;
  # example:  /usr/bin/media-ctl -d /dev/media0 -R "'Intel IPU7 CSI2 2'[0/1->3/1,0/0->1/0]"
  ...
  if [[ $metadata_enabled -eq 1 ]]; then
      ...
      out $media_ctl_cmd -R "\"Intel IPU7 CSI2 ${csi2}\"[0/${isys_vc_depth}->${isys_pad_depth}/${isys_vc_depth}[1], 0/${isys_vc_depth}->$((${isys_pad_depth}+1))/${isys_vc_depth}[1], 0/${isys_vc_rgb}->${isys_pad_rgb}/${isys_vc_rgb}[1], 0/${isys_vc_rgb}->$((${isys_pad_rgb}+1))/${isys_vc_rgb}[1], 0/${isys_vc_ir}->${isys_pad_ir}/${isys_vc_ir}[1], 0/${isys_vc_imu}->${isys_pad_imu}/${isys_vc_imu}[1]]"
  else
      # Set DEPTH+RGB+IR+IMU vc_id routes
      out $media_ctl_cmd -R "\"Intel IPU7 CSI2 ${csi2}\"[0/${isys_vc_depth}->${isys_pad_depth}/${isys_vc_depth}[1], 0/${isys_vc_rgb}->${isys_pad_rgb}/${isys_vc_rgb}[1], 0/${isys_vc_ir}->${isys_pad_ir}/${isys_vc_ir}[1], 0/${isys_vc_imu}->${isys_pad_imu}/${isys_vc_imu}[1]]"
  fi

  # DEPTH default media bus format
  out $media_ctl_cmd -V "\"Intel IPU7 CSI2 ${csi2}\":${isys_pad_depth}/${isys_vc_depth} ${fmt_depth}"
  ...
  out $media_ctl_cmd -V "\"Intel IPU7 CSI2 ${csi2}\":${isys_pad_rgb}/${isys_vc_rgb} ${fmt_rgb}"
  ...
  out $media_ctl_cmd -V "\"Intel IPU7 CSI2 ${csi2}\":${isys_pad_ir}/${isys_vc_ir} ${fmt_ir}"
  ...
  out $media_ctl_cmd -V "\"Intel IPU7 CSI2 ${csi2}\":${isys_pad_imu}/${isys_vc_imu} ${fmt_imu}"
